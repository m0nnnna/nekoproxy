#
# NekoProxy Agent Installer for Windows
#
# Registers the agent as a Windows service using the .exe in this directory.
# Run as Administrator. The service will use nekoproxy-agent.exe from the
# same folder as this script. Place agent.env (or .env) in the same folder
# for configuration.
#
# Usage: .\install-agent.ps1
...
if ($Uninstall) {
    ...
    Write-Host "  Uninstall NekoProxy Agent (Windows)" -ForegroundColor Cyan
    Write-Host "================================================" -ForegroundColor Cyan
    Write-Host ""
...
Write-Host ""
Write-Host "================================================" -ForegroundColor Cyan
Write-Host "  Install NekoProxy Agent (Windows)" -ForegroundColor Cyan
Write-Host "================================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "Using: $ExePath" -ForegroundColor Gray
$configPath = Join-Path (Split-Path $ExePath -Parent) "agent.env"
if (Test-Path $configPath) {
    Write-Host "Config: $configPath" -ForegroundColor Gray
} else {
    Write-Host "Config: agent.env not found in same folder; agent will use defaults or .env" -ForegroundColor Yellow
}
Write-Host ""
...
try {
    ...
    if ($StartService) {
        ...
        Write-Host ""
        Write-Host "To start the agent: Start-Service $ServiceName" -ForegroundColor Cyan
    }
} finally {
    Pop-Location
}
...
Write-Host "  Uninstall: .\install-agent.ps1 -Uninstall" -ForegroundColor Gray
Write-Host ""
